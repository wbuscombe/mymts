...
### Native app feed — agnostic-with-source vs per-source sections (OPERATOR'S CALL, web-rework r2)

**What:** The web client's feed was changed to an **agnostic chronological list** (one newest-first river, source next to each headline — the original Onn style) per the operator's hands-on preference after using both. The **native** app's feed is still **per-source sections** (Stage 7 feed-restructure, `FeedListBuilder`/`FeedPane`, commit a756184) — which the operator approved *then*. The question: should the native feed ALSO revert to agnostic-with-source-label, or keep sections?
**Why-not-now:** Deliberately NOT changed unilaterally. The native sectioning was a real prior chapter with a focus-model contract (headers are visual-only; `WallFocusModel` treats the feed as one flat list); an agnostic native feed is a focused change with its own test pass, not a side-effect of a web tweak. The web client diverging is fine — it's a per-client preference, and the helper serves both the same data. So the two clients can legitimately differ in feed *layout*.
**Reconsider when:** the operator decides they want the native wall's feed to match the web client's agnostic style. If yes: collapse `FeedListBuilder.build`'s grouping to a flat newest-first list with an inline source label per row, keep the flat-index focus contract, re-pin `FeedListBuilderTest`. A focused follow-on, not urgent.
