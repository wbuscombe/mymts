...
3. **~~NWS / NOAA radar~~ — DONE 2026-06-27.** Shipped as a selectable per-cell **Weather Radar** widget source: the free public NWS RIDGE animated loop GIF (`{SITE}_loop.gif`), helper-proxied + per-region cached (5-min TTL, rate-respectful), CORS-clean, honest last-good/offline fallback, region-configurable (US National + central-IL KILX + major metros). A **pseudo-channel** — it rides the existing per-cell picker (web-only via `?widgets=1`) + wall config with no schema change; the region is encoded in the slug. Non-video → no decode load. See CHANGELOG + ARCHITECTURE §33. Follow-ons: per-region live/offline status in the picker; a current-conditions data panel; more regions on request.

**Why-not-now:** (1) violates the keyless/no-credentials Trust Bar; (2) cosmetic — three nationals already cover it; (3) **done** (item 3, above).
**Reconsider when:** (1) only if a station ever exposes a public keyless HLS (re-probe periodically); (2) on operator request.
