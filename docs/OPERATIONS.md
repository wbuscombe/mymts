...
- **Panel fit (this panel overscans — 2026-06-07):** the box outputs a full 1280×720 but the panel cuts the outer edge, so wall content clipped. Fix is **in-app**, two D-pad levers in MENU → Settings (they lead the card): **"Display size"** (Compact/Default/Roomy — a global UI scale; pick **Compact** to shrink the whole wall to fit) and **"Overscan inset"** (None/3%/5%/7% — a safe-area margin; default **5%**; raise if still clipping, drop to **None** on a clean panel). Both persist on-device. Optional box/panel-side help: if the panel's own picture menu has "just scan"/1:1/aspect, that reduces the physical overscan so a smaller app inset suffices. (The HDMI output mode is correct; only the panel geometry overscans.)
