...
- **Panel fit (this panel overscans off-center, no hardware menu — 2026-06-07/-09):** the box outputs a full 1280×720 but the panel cuts the outer edge AND shifts the image, and the panel has no buttons to adjust itself — so ALL compensation is **in-app**, via **three** D-pad levers in MENU → Settings (they lead the card; tune by eye while watching the panel):
  1. **"Display size"** — global UI scale (Compact/Default/Roomy). Pick **Compact** to shrink the whole wall if it's too big.
  2. **"Overscan inset"** — safe-area margin (None/3%/5%/7%, default **5%**). Raise if still clipping; **None** on a clean panel.
  3. **"Position X" / "Position Y"** — nudge the whole wall to **recenter** it (LEFT/RIGHT adjust live, ±64 dp). Scale + inset are symmetric and can't recenter a shifted image; this does.
  Suggested starting point for this panel: **Display size = Compact**, **Overscan = 5–7%**, then nudge **Position X/Y** until the wall is centered + fully inside the visible area. All persist on-device. (The HDMI output mode is correct — 720p; only the panel geometry overscans/shifts. If the panel ever gains a "just scan"/1:1 picture setting, that reduces the physical overscan so smaller app values suffice.)
