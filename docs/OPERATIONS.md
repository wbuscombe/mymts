...
### Whole-ticker paged flip + sports news in the feed (2026-06-11, helper `c717e7e`)

- **Ticker:** the WHOLE ticker is now a paged flip — market quotes are bordered cards (scrolling if they overflow), each sports league is a page, and the strip flips between them all with one motion (markets flips in like a league block). Markets quotes are **LIVE** as of 2026-06-11 (Yahoo Finance, NAS-reachable — see the markets-live note above); SAMPLE only shows for a symbol whose fetch genuinely fails.
- **Sports news in the feed:** the 8 ESPN keyless RSS news feeds are seeded as feed sources (label = league). They appear inline in the agnostic feed, **gated by the SAME "Sports leagues…" filter** as the ticker scores — disable a league and its scores AND its news both disappear. Capped to the newest 14 so they blend, not flood. To tune the mix: disable leagues in MENU → Settings → Sports leagues…, or change `FeedListBuilder.MAX_SPORTS_NEWS`.
- **`.92` deploy note:** the box's Wi-Fi ADB transport is now **truncating/corrupting `adb push`** (reports success but lands a partial/0-byte file). Deploy must **verify the exact on-device byte size matches the local APK before `pm install`** (retry the push until `stat -c%s` equals the local size); installing a truncated APK fails. Several push retries were needed this deploy.
...
**✅ Markets ticker — LIVE from the NAS (2026-06-11, was SAMPLE).** Previously, Stooq served a JS anti-bot challenge to the NAS egress IP so indices/FX/gold fell back to honest SAMPLE pills. **Fixed:** the helper now fetches **Yahoo Finance's keyless v8 chart endpoint** (`query1.finance.yahoo.com/v8/finance/chart/<symbol>`), which **is reachable from the NAS egress** (validated from inside the helper container — the prober is the gate, the WeatherNation lesson). All 14 quotes are now live — indices, FX, gold, AND the three that used to be sample-only (Brent/WTI/10Y UST); crypto stays on CoinGecko. **Keyless** (no key, no `.env`). The honest SAMPLE fallback remains for any symbol whose fetch genuinely fails this cycle (per-symbol — one symbol failing doesn't sample the rest). Verify: `curl -sk https://<LAN_IP>:8443/api/ticker/markets | jq '[.entries[] | {symbol, display, is_sample}]'` — expect `is_sample:false` for the real symbols.
...
- **Panel fit (this panel overscans, no hardware menu — 2026-06-07/-10):** the box output is geometrically perfect (full 1280×720, `scale=1.0`, no software overscan) — the **panel** physically crops the frame, rendering the wall **larger than its visible area from a top-left origin** (top-left seated, bottom-right overflowing). All compensation is **in-app**, via D-pad levers in MENU → Settings (LEFT/RIGHT adjust live; tune by eye while watching the panel). **Working values for `.92`'s panel: Fit scale = 80%, Vertical stretch = 110%** (everything else default). The levers, in order:
  1. **"Fit scale"** (50–100%) — the primary lever here. Uniform shrink of the whole wall **anchored at the TOP-LEFT corner**, so the overflowing bottom-right pulls into view while the top-left stays pinned. LEFT = smaller. (Use this, not Overscan inset, when the panel is top-left-anchored — the inset shrinks toward *centre*, the wrong way.)
  2. **"Vertical stretch"** (100–130%) — height-only grow on top of Fit scale (top-left anchored) to close a residual **bottom band** without moving the sides. Slight aspect distortion; default 100%.
  3. **"Display size"** — global UI scale (Compact/Default/Roomy); resizes chrome, **not** the footprint (so it doesn't help fit — readability only).
  4. **"Overscan inset"** (None/3/5/7/10/13/16/20%, default 5%) — symmetric safe-area margin, shrinks toward *centre*. For a centred overscan, not this top-left one.
  5. **"Position X" / "Position Y"** (±64 dp) — nudge the whole wall to recenter a *shifted* image.
  6. **"Calibration border"** (default off) — diagnostic: a magenta boundary + cyan TL/TR/BL/BR corner brackets at the wall edge so you can SEE which edges the panel crops. Turn on to dial fit, then off.
  (HDMI output is correct — 720p. The panel is SD-ish and overscans HD; Android won't emit sub-720p and there's no root/sysfs overscan knob on this Google TV build, so the fix is app-side. A panel "just scan"/1:1 setting, if it ever has one, would let you drop these to defaults.)
- **Wall changes (2026-06-11):** **default 2×2** is now TL LiveNOW from FOX · TR Fox Weather · BL BBC News · BR CBS Sports HQ (reassign any cell via the picker; your explicit choices still win). **PGA** ticker card shows the **top 10** and scrolls horizontally. **Ticker speed:** MENU → Settings → **Ticker** → *Scroll speed* / *Flip speed* sliders (LEFT/RIGHT, 40–200%, the flip is calmer by default now). **Manual reconnect** for a drifted feed: focus a tile → SELECT → **Reconnect** (one tile), or MENU → Settings → **Refresh all video** (all tiles). **Channel picker** is grouped by category (Sports / US News / Global News / Business / Weather / General). *(Pending: a news-feed genre/source toggle tree — checkpointed, see BACKLOG.)*
- **Settings menu grouped into sections (2026-06-11):** MENU → Settings is now organized under **Display & Fit** / **Layout & Feed** / **Sports** headers (was one long flat list). Same D-pad nav — open with **LEFT** (the side menu; Settings is the WALL-section row below the channel slots), UP/DOWN through rows, LEFT/RIGHT (or SELECT) to change a row; the card **scrolls and follows focus** so nothing clips even with a 6/9 grid pushing it down. Purely a reorg — **all values preserved**, the locked fit included (verified: Fit 80% / Stretch 110% / Overscan None / Pos 0,0 / Calibration Off). The **Sports leagues…** picker lives in the Sports section (8 leagues, all on by default, drives ticker scores + sports-news) and lists the staged sports as *"Coming soon: UFC · PGA · Tennis · F1"* (they become toggles when their cards ship). *(Note: BACK out of an overlay returns to the menu; pressing BACK repeatedly past the menu exits MyMTS to the Google TV launcher — relaunch via the launcher or `am start`.)*
- **Video grid — Rows × Columns (menu setting, 2026-06-11 update):** MENU → Settings → **Layout & Feed** has two rows — **"Grid rows"** and **"Grid columns"**, each **1–3** (LEFT/RIGHT to adjust), **default 2 × 2**, persisted. So 2×2, 2×3, 1×3, 3×2 … up to 3×3 (=9). Cell count = rows × cols; the grid re-flows and your per-slot channel choices survive. *(Replaces the old single "Video grid" 1/2/4/6/9 count — the `gridSize` pref was dropped, so the grid reset to 2×2 on this update; re-set rows/cols if you'd changed it.)* **Channel assignment is now a scrollable LIST** (MENU → focus a slot → SELECT → Channel): D-pad UP/DOWN through all channels, SELECT to assign — opens on the slot's current channel, LIVE/OFFLINE grouped. The old single-count note follows for history:
  - *(historical)* Old behavior: **"Video grid"** cycled **1 / 2 / 4 / 6 / 9** tiles (LEFT/RIGHT or SELECT), **default 4 (2×2)**. The layout is grid-agnostic: each tile is a [video + label] unit (video letterboxed, channel label in a strip **below**), and the section reserves a bottom safe-band so the lowest row's label never clips — at any count. Changing the count keeps your explicit per-slot channel assignments (default fill is a stable prefix, so 4→6→4 restores the same lineup). Independent of the panel-fit levers above (those are read-only here, untouched). *(Tile budget: the soak-gated sustained budget is 4 on this box; 6/9 are available but un-soaked — watch for DEAD tiles if you run dense grids long-term.)*
...
- **Apps installed:** **MyMTS** release-signed via `scripts/deploy-app.sh --device <LAN_IP>:5555` — health-gate **PASS (119 `EV=TILE_READY`, 0 dead)**, promoted known-good `mymts-0.0.0+1b8d1b0`. **WyzeGrid** built debug + installed **dormant** (no kiosk flag, not launched; debug key `~/Dropbox/secrets/wyzegrid-debug.keystore`). One-kiosk-per-box (Model A) preserved.
  ...
  - **`.92` deploy gotcha (flaky Wi-Fi ADB transport, 2026-06-10):** streamed `adb install` **deadlocks** on this box (hung 53 min once). Reliable path: **`adb push` the APK to `/data/local/tmp`, then `pm install -r -t` the local file** (the install reads a local file, so no sustained transfer to wedge), and **verify `dumpsys package com.mymts | grep lastUpdateTime` actually advanced** — `pm install` sometimes reports `Success` without committing when the transport drops. The transport wedges intermittently on sustained transfers (a push can land a **0-byte** file); recover device-specifically with `adb disconnect/connect <LAN_IP>:5555` (NEVER `kill-server` — protects `.182`) and retry. Wrap each op in a timeout (`perl -e 'alarm N; exec @ARGV' adb …` — macOS has no `timeout`).
...
- **§6 reboot test — partial, one known gap:** `adb reboot` → **network ADB survives** (re-arms on boot despite empty `persist.adb.tcp.port`); box **auto-restores 720p + screen Awake**; `BootReceiver` fires → `KioskService` foreground starts on boot. **BUT the wall ACTIVITY does not auto-foreground over the Google TV launcher** — `KioskService.launchWall()`'s boot-time `startActivity` is blocked by Android 14 **background-activity-launch (BAL)** restrictions, so the launcher stays in front and 0 tiles render until the wall is opened. **The wall works perfectly when launched** (manually or by selecting it).
  ...
  - **Full-screen-intent (code-only) ATTEMPTED 2026-06-07 — fired but did NOT foreground the wall on this Google TV build; reverted.** Added a `USE_FULL_SCREEN_INTENT` permission + a HIGH-importance channel + `KioskService.requestWallForeground()` posting a `setFullScreenIntent(...)` notification from the `BootReceiver` boot path (the Android-sanctioned background→foreground mechanism; touches NO launcher). Granted the app-op (`appops set com.mymts USE_FULL_SCREEN_INTENT allow`). Reboot telemetry confirmed it fired correctly — `canUseFullScreenIntent=true`, `requestWallForeground | full-screen-intent posted` — **but the wall still did not come to the foreground** (launcherx stayed in front, 0 tiles). On the TV form factor the system treats a full-screen-intent as a notification, not an auto-launch. **Reverted** the code (manifest + KioskService + BootReceiver) + the app-op; redeployed the clean APK (promoted `5975c36`); box back to clean known-good. Tests stayed 246.
  - **DECISION (2026-06-07): wall-on-boot = "accept manual launch" for now; device-owner/lock-task is the logged future path.** A researched, deliberate call — not an open loose end. **Current state:** the box boots to the Google TV launcher at 720p; **open MyMTS once → the wall runs LIVE + the kiosk holds it foregrounded.** Only matters on the rare reboot (dedicated appliance on stable power), so the manual-launch cost is small. The robust "true boot-to-wall" path is **device-owner / lock-task** (`dpm set-device-owner` on a freshly-reset box) — but it's a **deliberate future project**, not a reactive try: it requires a **factory reset** (wipes the debloat + both app installs + config → full re-provision), the app then **can't be uninstalled** without another reset, `set-device-owner` is **documented to sometimes fail on TV hardware**, and it needs a `DeviceAdminReceiver` built into MyMTS first. Full context + the two ruled-out approaches (HOME-launcher, full-screen-intent) are in `docs/BACKLOG.md §"Wall-on-boot (auto-foreground)"`. Do **NOT** retry the launcher-disabling path — it destabilizes this build.
...
6. **Confirm the wall reaches the helper.** On the box, the wall launches automatically (LEANBACK launcher). Verify: feed pane populates (13 sources sectioned), at least one video tile reaches **LIVE**, the ticker shows real markets data (all live via Yahoo Finance as of 2026-06-11; SAMPLE only on a genuine per-symbol fetch failure). If the feed/tiles stay empty, re-check the helper prerequisite above and that the box can reach `<LAN_IP>:8443`.
