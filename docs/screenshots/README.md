...
| `control.png` | The **picker control surface** (`/control/`) — the headless-container version's wall editor. The wall's layout in a browser, but **each cell is a feed-PICKER** (a category-grouped channel dropdown, incl. the Weather Radar widget) + per-cell **Audio** (multi-audible — any subset of tiles can be unmuted) + **Subtitles** toggles, plus grid rows×cols + preset selectors and the Display-tuning sliders (resolution — an 8-rung 720p–2160p ladder — feed width, feed font, ticker height). **Plus the Outputs fan-out panel** ("one wall, many streams"): an **HLS/VLC** card (resolution/bitrate/audio + start/stop), an inert **Mercury** LiveKit-publisher card (a needs-setup checklist — LiveKit key / tailnet / channel), and a launch-to-start **Discord** Activity card (needs-setup until configured) — each honest about its state, never faking "connected/publishing". **No video decode** — it runs on a phone; every change writes the server-side wall config that `/app/` renders from. |
