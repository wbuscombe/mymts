...
| `channel-picker.png` | The channel picker (opened from a slot's Channel row) — the channel lineup **sectioned by category** (Sports / US News / Global News / Business / Weather / **Weather Radar** / Cameras / Government / Nature / Space / General) with the honest **live / on-the-TV-wall-only / offline** legend. The free **government** feeds (chamber floors, live committee hearings, federal agencies, the White House feed) live in their own **Government** section so US News reads live-dense; **Weather Radar** is a web-only widget category (free public NWS radar loops, selectable per cell). |
...
| `control.png` | The **picker control surface** (`/control/`) — the headless-container version's wall editor. The wall's layout in a browser, but **each cell is a feed-PICKER** (a category-grouped channel dropdown, incl. the Weather Radar widget) + per-cell **Audio** (single-audible) + **Subtitles** toggles, plus grid rows×cols + preset selectors, **plus a Display-tuning panel** — resolution (an 8-rung 720p–2160p ladder), feed width, feed font, and ticker height sliders. **No video decode** — it runs on a phone; every change writes the server-side wall config that `/app/` renders from. |
