...
| `channel-picker.png` | The channel picker (opened from a slot's Channel row) — the channel lineup **sectioned by category** (Sports / US News / Global News / Business / Weather / **Weather Radar** / Cameras / Government / Nature / Space / General) with the honest **live / on-the-TV-wall-only / offline** legend. The free **government** feeds (chamber floors, live committee hearings, federal agencies, the White House feed) live in their own **Government** section so US News reads live-dense; **Weather Radar** (free public NWS radar loops, selectable per cell) is served to **every** surface — native TV **and** web — via the unified registry (no `?widgets` gate). |
...
| `control.png` | The **picker control surface** (`/control/`) — the headless-container version's wall editor. The wall's layout in a browser, but **each cell is a feed-PICKER** (a category-grouped channel dropdown, incl. the Weather Radar widget) + per-cell **Audio** (multi-audible — any subset of tiles can be unmuted) + **Subtitles** toggles, plus grid rows×cols + preset selectors and the Display-tuning panel — **four independent 1–10 step sliders** (feed width, feed text, ticker height, ticker text; step 5 = the default, and each shows its step alongside the value it resolves to). Ticker height and ticker text are separate knobs, so a tall bar with small text is expressible. Changes **apply live on the rendered wall** — no reload, no interruption to playback. (The render resolution now lives on the Outputs cards, not here.) **Plus the Outputs fan-out panel** ("one wall, many streams"): an **HLS/VLC** card (resolution/bitrate/audio + start/stop) — honest about its state. **No video decode** — it runs on a phone; every change writes the server-side wall config that `/app/` renders from. |
