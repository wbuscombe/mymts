...
### Versions (matched to a sibling Android-TV app's validated stack)
...
└── src/main/java/com/mymts/
    ...
    ├── util/CrashLog.kt         # file-backed event/breadcrumb log (mirrors a sibling TV app)
...
- `app/StreamSpec` rejects `rtsp://` and `file://` schemes — guards against accidental copy of a sibling camera app's plumbing.
...
## 6. Sibling-app pattern reuse — and the hard boundary
...
- **`network_security_config.xml` for cleartext-to-local-bridge.** The sibling camera app needs it to talk to `<LAN_IP>`; MyMTS does not (the helper terminates inside a private docker network the TV never reaches directly).

The boundary is enforced by code (`StreamSpec` rejects non-http schemes), by test (`StreamSpecTest`), and by review discipline: if a future change finds itself reaching into a sibling camera app's Frigate / camera / weather APIs, that change is on the wrong side of the boundary.
...
| Endpoint | Returns |
|---|---|
| `GET /api/wall` | the current wall config (or a server default when none stored), tagged `{…, stored}` — `{schema_version, layout:{rows,cols}, preset, reload_epoch, feed_pct, feed_font, ticker_scale, outputs:{hls}, cells:[{channel, audio, subtitles, reload}], stored}` (§34: per-cell `audio` + the `outputs` map; the old `render`/`audible_cell` migrate on load) |
| `PUT /api/wall` | **partial-merge (PATCH)**: a field PRESENT in the body overrides; an OMITTED field is PRESERVED from storage (so `/app/` can't clobber a field it doesn't send). The merge deepens to per-output AND per-field (`outputs.hls.bitrate_kbps` alone never clobbers `outputs.hls.resolution`). Validation on the MERGED result; reload + per-output `restart_epoch` counters clamped monotonic; `422` with a field-specific reason. See §30/§34 |
| `GET /api/outputs/status` | per-output runtime state (the config's outputs merged with the renderer's status file, path-safe). HLS: `running`/`stopped`/`disabled` + res/bitrate + playlist path. See §34 |
| `POST /api/outputs/{hls}/{start\|stop\|restart}` | a control edit through the SAME validated merge path as `PUT /api/wall` — start/stop flip `enabled`, restart bumps the monotonic `restart_epoch`. `404` on an unknown output/action |
...
1. Operator's preferred slugs (`cbs-sports-hq`, `bbc-news`, `cnn`, `livenow-fox`) — picked first.
...
3. Any remaining channels — top up so the grid isn't half-empty when other channels work.

The selector is **pure**: it ranks whatever channel list it is handed, and the caller decides
whether that list is filtered. The **default wall** calls `LineupSelector.defaultWallLineup`,
which runs this same walk over the helper's **full** channel set (live or offline), so
**policy A** holds: a configured channel that is down keeps its *original* slot and renders
the existing honest C2 OFFLINE tile (`TileSlotResolver.Slot.Offline`, labelled with its
name), live channels never shift up to close the gap, and no live channel is repeated into
it. A **top-up preset** is handed the playable subset instead, so there a slug that stops
resolving does drop out. `DENY` (`nasa-tv`) and the radar exclusion apply exactly as before.
...
The wall gains a sibling-app-style left side panel for **channel/lineup control only**. Settings, layout config, diagnostics, and other future menu rows are explicitly deferred.
...
└── ui/menu/
    ...
    └── MenuColors.kt            ← sibling-app palette (green focus, dark translucent)
```
...
| `Slot.Offline(idx, channel)` | the slot's channel exists in helper but `status != live` (mark-and-allow) — an operator pin, or a default-lineup channel that is down (policy A) | C2 honest panel **with the channel's label** |
...
This is the "safe excerpt is the v1 answer; richer reading is deferred" principle (an early A1 closed-door design decision). It satisfies A1 while keeping the feed interactive. No compromise. Adversarially verified in `docs/THREAT-MODEL.md §"Navigation chapter — feed-expand A1 confirmation"`.
...
The dedicated MyMTS Onn box arrives today. This stage builds the kiosk, foreground, and boot scaffolding that does not need the physical device — on-hardware validation is explicitly staged for the migration session. **Model A**: one kiosk app per box. The new box runs MyMTS as the sole kiosk; a non-kiosk box on the LAN stays untouched.

### Load-bearing safety property: opt-in, off by default

Kiosk mode is **opt-in and disabled by default**. The same signed APK on a non-kiosk box (notably where a MyMTS dev install may linger) must not autostart on boot or launch a foreground service. Only the provisioning runbook flips it on for the dedicated box via `adb ... --ez kiosk true`.

This gate eliminates the Stage 1/2 risk: two watchdogs thrashing over foreground-service reclaim. Model A says "own the box, stay foreground reliably" — not "coexist and reclaim." The opt-in flag is the load-bearing property that makes this safe, and is what guarantees the shared APK does nothing kiosk-ish on a non-kiosk box.
...
All policy logic is pure and deterministic — 7 `KioskPolicyTest` cases cover the boot allowlist, the both-conditions gate (the non-kiosk-box safety property), the relaunch decision, and the backoff schedule + streak window. So the device session validates *wiring*, not *logic*.

**Not verified until the box arrives** (and honestly labelled so): the service actually holding the foreground across hours; the boot receiver relaunching via a real power-cycle; low-memory survival; the full runbook end-to-end; and the accumulated nav/feed/config/ticker feel-test (now on the MyMTS box rather than a borrowed dev box). The helper redeploy (13 feed sources + ticker endpoints) is a migration prerequisite.

### What's deliberately NOT here

- Coexistence / foreground-reclaim (Model A makes it unnecessary); anything on a non-kiosk box.
...
- **Ticker motion is one cross-platform setting (Part 3).** Both motions now
  ...
  next on-device release, the same as any native-wall change.
...
On the dedicated box's 720p panel, two things surfaced. **(1) Panel fit.** The box outputs a clean 1280×720 @ density 213, but the *panel physically overscans* (cuts the outer edge) where the a sibling TV app box's panel doesn't, and modern Android removed the software overscan knob (`wm overscan`). The fix is app-side + panel-agnostic, via two `WallSettings` presets persisted in `LineupStore` and cycled from Settings ("Display size" + "Overscan inset" rows lead the card):
...
**Fresh-source pass — international news restored + CBS News 24/7 (2026-08-18).** Lineup **52 → 56**, server-authoritative, **no app rebuild** (every addition lands in an existing section, so `CATEGORY_ORDER` is byte-identical across all three surfaces and the native picker groups them via `sectionedByCategory`). **Al Jazeera English**, **CGTN English** and **TRT World** are **back**: the 2026-06-22 prune removed them because their old *direct-HLS* origins were persistently dead (~1000 consecutive dns/SSL failures, hosts gone) — that was **superseded-endpoint** evidence, not a judgment on the broadcasters, so each is re-sourced to its **official YouTube `/live` handle** (`kind='youtube'`) and rides the same `is_live`-gated yt-dlp resolver → honest-offline as every other YouTube channel. The seed pins the **@handle**, never a rotating video id, so a restream can't strand the tile (pinned by a test). **CBS News 24/7** joins US News on its **first-party token-free direct-HLS origin** (`kind='hls'`). Every addition was validated through the project's own credential-free path — **master → variant → one real segment**, twice, several minutes apart — with no auth, no cookies, no `Referer`, no DRM (`#EXT-X-KEY` absent) and no per-play signing.

*Honest-nos from the same pass.* **CBS News Chicago** (the first-party market feed): its master manifest still serves 200 with a full six-rung ladder, but **every advertised variant returns `404`** — the master-OK / variant-FAIL shape the prober's deepened validation exists to reject; identical on both probes. The only other Chicago path is Pluto's per-play-JWT ad-stitcher, already refused in finding 23. **NASA TV** (`nasa-tv`, the existing NTV1 direct-HLS entry) re-probed the same way: master 200, **all three variants 404** — the standing `DENY` + honest-offline is confirmed correct and it stays a re-source candidate. The generic **EarthCam** and **NASA** entries stay labelled as the **broadcaster handles** they are: both resolve live, but their `/live` handle rotates through programming (EarthCam happened to be showing Wrigley Field at probe time), so relabelling either to whatever was on screen would be a claim the source cannot sustain — see finding 24.

**Desktop executable (`tools/desktop/`, released v0.1.1).** The helper packaged as a launchable desktop app (PyInstaller) so a user runs the **web wall** with no clone / Docker / Python: a **menu-bar / system-tray** app (macOS/Windows) that serves the helper on `http://127.0.0.1:PORT` (localhost — a browser secure context, no TLS) and opens the wall; **headless server mode** on Linux / no-display. It reuses the helper's `create_app()` (no new server logic), keeps the seeded DB in a per-user data dir (never the read-only bundle), and runs a **yt-dlp self-update** — fetch the latest from PyPI's official hosts over HTTPS, **SHA256-verify** against PyPI's digest, and prefer it over the frozen copy via a `meta_path` finder ahead of PyInstaller's `FrozenImporter`, with an offline/launch-safe fallback — so the YouTube channels stay current independent of app releases. The desktop builds ship **unsigned**: no signing or notarization runs in CI, and the release body carries the one-time Gatekeeper right-click→Open and SmartScreen Run-anyway notes. A tag-triggered GitHub Actions matrix builds the unsigned macOS/Windows/Linux artifacts and creates the GitHub Release as a **draft**. CI builds **no Android APK**: `mymts-<version>.apk` is built with `assembleRelease` and signed **locally** against a keystore deliberately kept off CI, confirmed with `apksigner verify`, and attached to the draft by hand. The draft is published only after that APK is attached and verified. The gradle signing reads the keystore from the gitignored `keystore.properties` or from `MYMTS_RELEASE_*` environment variables; no CI job supplies either. The bundle/APK are gitignored; only the tooling/workflows are committed. See `CHANGELOG.md` + `SECURITY-PRACTICES.md` + `docs/DOWNLOAD-AND-RUN.md`.

---

## 26. Headless container version — server-side wall config + picker control surface (2026-06-23)

The **headless NAS-container version** of the wall: instead of a person watching a browser, a headless renderer (next phase) will stream the rendered `/app/` wall to a generic player (VLC / Apple TV). A headless wall has **no device** to hold its lineup, so the wall's state moves **server-side**. First half (this section): the server-side wall config + a lightweight picker control surface that writes it + wiring `/app/` to render from it. Helper + web only — no native, no on-device build.
...
> **Superseded in part by §41 (2026-07-09).** §31's headline "1080p output is 300/300 unique frames — already smooth" was measured with `mpdecimate` on the composited HLS output. That metric is **ticker-dominated and blind to per-tile decode loss**: the encoder emits a perfect 30 CFR by duplicating, and a whole-frame unique-frame count cannot see an individual `<video>` tile decoding at a fraction of its rate. §41 instrumented the missing number (per-tile `getVideoPlaybackQuality`) and found that at 1080p the news tiles were dropping **11–40% of frames each** — a real, distinct choppiness §31 could not detect. §31's real-time-encode tuning and the *resolution* envelope below still stand; its "1080p is already smooth" conclusion is corrected by §41's per-tile fix.
...
> **Superseded in part by §44 (2026-08-03).** §32's *resolution ladder* still stands in full. Its **feed/ticker tunables paragraph below is superseded**: the three continuous fields (`feed_pct` / `feed_font` / `ticker_scale`) are retired in favour of **four** 1–10 integer steps in a `feed` + `ticker` block, and the claim that `--ticker-scale` drives *every* ticker dimension **including the card text** no longer holds — ticker height and ticker text are now independent knobs (`--tu` / `--ttu`). The paragraph is kept as the record of what shipped in 0.4.0. See §44 for the current model.
...
## 36–40. Discord Activity + Mercury/LiveKit outputs — REMOVED (PR-018, 2026-07-17)

These sections covered a **Discord Activity** viewer output (a launch-to-start iframe that
*viewed* the one HLS render — no second encode) and the **Mercury/LiveKit** screen-share
publisher (its Cloudflare-tunnel origin, the `/.proxy/` routing fix, and the cache/failure
hardening). Both lanes were removed in **PR-018**: the Discord output was a near-zero-cost
HLS viewer, and the Mercury publisher never ran past its inert stub (no LiveKit credentials
ever arrived). The general **capture-once → fan-out** framework they plugged into survives
unchanged (§34); only the two integration entries are gone. Section numbers §41+ are kept
stable (code + CHANGELOG cite them). See the CHANGELOG for the full removal record.

## 41. Renderer — true per-tile smoothness: variant-vs-cell overdraw, measured + fixed (2026-07-09)

§31 made the ENCODE real-time and characterised the *resolution* envelope, and concluded 1080p was "already smooth" from a whole-frame `mpdecimate` count. This chapter instrumented the number that count is blind to — **per-tile decode** — found the real 1080p judder, fixed it, and left a reusable harness.

**The blind spot.** The encoder emits a perfect 30 CFR by duplicating, so `mpdecimate` on the HLS output cannot see an individual `<video>` tile decoding slowly — and on a multi-tile wall it is doubly blind: per-tile motion is a *fraction* of the frame (below mpdecimate's `frac` threshold) and H.264 smooths sub-threshold motion, so the output unique-frame count reads **3–18 fps even when tiles move every frame** (measured). The authoritative signal is per-tile `getVideoPlaybackQuality()` (decoded vs dropped frames) + `requestVideoFrameCallback` (presented frames), exposed by an opt-in `?fpsmeter=1` render-page overlay that POSTs to a LAN-only `/api/render/telemetry`, scraped by `tools/render-bench/` (which measures **every** stage at once — per-tile decode/drop, variant-vs-cell, page paint, encoder unique, per-process CPU).

**Root cause — variant-vs-cell overdraw.** Each wall cell paints into ~700 device-px, but hls.js ABR loaded each stream's FULL-RES variant, so the GPU-less renderer software-decoded **far more pixels than it displayed**, N tiles at once, and dropped frames. Measured on the live 2×2 wall (3 video tiles + a radar widget), **60s windows**:

| Tile (source fps) | Loaded variant | Cell (dev-px) | Overdraw | Drop % (before) |
|---|---|---|---|---|
| LiveNOW (30) | 1920×1080 | 703×463 | **6.4×** | **11 %** |
| BBC News (50) | 1280×720 | 703×463 | **2.8×** | **33 %** |
| CBS Sports (60) | 1920×1080 | 703×463 | **6.4×** | **40 %** |

Page paint was healthy throughout (rAF 50–60 fps; framebuffer `x11grab`+mpdecimate ~29 fps) and the encoder had headroom (30 CFR, ~1.1 cores) — so the loss was **purely per-tile software decode**, not the compositor and not the encoder, and not content stillness (still content generates no frames to drop; these tiles decoded frames and *then* dropped them).

**The fix — `capLevelToPlayerSize: true` (`video.mjs`).** hls.js now caps each tile's level to its element size, so a ~700px cell loads a ~480p rendition instead of 1080p — ~5× less decode. It is size-driven, so a maximised/fullscreen tile or the full-window laptop `/app/` still gets a high-res variant automatically (no special case). After (same wall, 60s, steady state):

| Tile | Overdraw (after) | Drop % (after) | vs before |
|---|---|---|---|
| LiveNOW | **1.3×** | **0 %** | 11 → 0 |
| BBC News | **0.9×** (480p/25fps rendition) | **0 %** | 33 → 0 |
| CBS Sports | **1.0×** | **0–5 %** (occasional 15% spike on the 60fps source) | 40 → ~0 |

Chromium decode CPU roughly halved (≈165 % → ≈70–105 % of a core-second). **No Chrome-flag change was needed** — the compositor was already healthy — so the renderer image is untouched (web-only fix; no chromium rebuild, the 147 pin holds).
...
**Honest sustainable envelope (this NAS, 1080p canvas, capped variants).** 3 live video tiles at ≤480p decode with <5 % drop while the wall also runs its single 1080p HLS fan-out encode (Discord, when it was configured, *viewed* that same HLS — it never added a second encode, per README); a 60fps source (CBS) is the hardest and occasionally spikes to ~15 % under the encode load. The `mpdecimate`-on-output number is **not** a smoothness metric for a multi-tile wall (see the blind spot above) — use `tools/render-bench/` (per-tile telemetry) for any future smoothness question. The remaining ceiling above 1080p is unchanged (§31 GPU envelope). The harness + the `?fpsmeter=1` instrumentation are opt-in and inert in prod.
...
**Concurrent-viewer headroom (helper `mem_limit` 384m → 512m).** A "two VLC viewers crashed something" investigation found the constraint was the helper's 384m ceiling itself, not a leak: `memory.current` sat within a few MiB of the 384 MiB cap (~99%), yet 1–4 concurrent `/stream` readers showed **0 errors, 0 restarts, memory flat** — a working set that fits comfortably under a slightly higher cap, not an anonymous/unreclaimable heap. Raised to **512m** (still a hard ceiling — the helper can never starve the host or its other services). A stateless-concurrency regression test (`helper/tests/test_stream_api.py`) fires **72 simultaneous in-flight `/stream` requests** and asserts each gets its correct, independent response (the passthrough is stateless — no cross-request contamination under load). Watch item: a slow creep toward 512m would flip the reading to a real leak.

## 44. Fine-grained feed/ticker sizing — four 1–10 steps + live-apply layout ([Unreleased], 2026-08-03)

Supersedes the feed/ticker half of §32 (the resolution ladder there is unaffected). Web + helper only — **`app/` is untouched**, so this stays in CHANGELOG `[Unreleased]` with no tag, per the no-tag-unless-`app/`-changes convention.

**What was actually wrong.** Recon corrected two assumptions worth recording. (1) The web controls were **not** a 3-step enum — they were already continuous floats; the 3-step feel is the **native** app's (`WallSettings` `FeedWidth` / `FeedFontScale` are deliberate 3-preset enums, and native has no ticker-size control at all). The real problems were that the *range* was narrow at both ends and that **ticker text was not separable from ticker height**. (2) The three tunables did already apply live on the rendered wall; what behaved like a force-reload was a **`layout.rows`/`cols` change**, which ran `buildGrid()` → tore down every `<video>` and restarted every stream — even for a 2×3 → 3×2 transpose with the same cells.

**Four independent controls, as 1–10 integer steps.** `feed.width_scale`, `feed.text_scale`, `ticker.height_scale`, `ticker.text_scale`, each an integer 1–10 defaulting to **5**. The wire carries the *step*; each step resolves through a 10-rung **ladder** of realised CSS values held in `store.py` and mirrored in `wallConfig.mjs` (the same contract the resolution ladder has with the renderer). Steps — not floats — because the value that matters is the rung, and a discrete scale is what survives being nudged from a phone; it also moves the web *toward* the native philosophy rather than further from it.

**Step 5 is exactly the pre-PR-024 default on every control** (32 % / 1.0× / 1.0× / 1.0×), so a migrated wall renders identically and every committed wall screenshot stays valid.

**The endpoints were derived from what renders, not chosen as round numbers.** Each candidate was driven through the real config path (`PUT /api/wall` → the wall's own 5 s poll → CSS) at 1920×1080 **and** 1280×720, then measured and looked at:

| Control | Step 1 | Step 10 | Was | The measurement that set the endpoint |
|---|---|---|---|---|
| `feed.width_scale` | **12 %** | **64 %** | 18–58 % | `.feed-pane { min-width: calc(220 * var(--u)) }` is **11.46 %** of the wall at every resolution. Below ~12 % the slider's bottom rung is a **silent no-op** — the operator moves it and nothing happens (measured: a request for 10 % renders at 11.46 %). At 64 % a 3×3 tile is still 218 design px with no chip collision. |
| `feed.text_scale` | **0.60×** | **2.00×** | 0.7–1.6× | 0.60× → 8.4 design-px headline, legible. **0.50× was rendered and rejected** (7.0 px headline / 4.5 px timestamp). |
| `ticker.height_scale` | **0.45×** | **2.75×** | 0.6–2.0× | Step 1 → an 18 design-px bar (1.8 % of the wall); step 10 → 110 design px = **10.2 %**, sized to stay a strip rather than swallow the wall. |
| `ticker.text_scale` | **0.55×** | **2.75×** | *(did not exist)* | The **least aggressive low end of the four, deliberately**: the ticker's smallest chip is 8 design px at step 5, already at the legibility floor. **0.50× was rendered and rejected** — the SAMPLE chips stop reading as text at 4.0 design px. |

The two ticker ladders are **identical from step 4 up**, so matching steps reproduce the old single-unit proportional feel (taller bar ⇒ bigger text) exactly; they diverge only at steps 1–3, where the text has a measured legibility floor the box does not.

**Honest limit at 720p.** `--ux` normalises design px, so the *design* geometry is resolution-independent — but real device px are `× 0.667` at 720p. Feed text at step 1 is 8.4 design px = **5.6 real px** there, and the ticker's SAMPLE chip is 2.9. The low steps are usable at 1080p (the default) and are a squint at 720p; that is a property of the canvas, not of the ladder, and it is why the low ends are conservative.

**Decoupling ticker text from ticker height (`--tu` / `--ttu`).** `--tu` (box) keeps bar height, gaps, paddings, radii; the new `--ttu` (text) takes all 21 `font-size` declarations **and their `letter-spacing`** — tracking is a glyph metric, so it must track the glyphs — plus `.ticker-mode-tag`'s `min-width`, which reserves a slot for its own label (on the box unit an oversized label would silently steal width from the crawl runway). **The load-bearing correctness change is that `.ticker-bar` uses `min-height`, not `height`:** with the units separated, text larger than its bar would otherwise be clipped **symmetrically** by `.ticker-viewport { overflow: hidden }`, shaving ascenders *and* descenders — which reads as a font bug, not a sizing mistake. With `min-height` the bar grows to contain its content instead. So **ticker height is a floor, not a cap**: at height step 1 with text step 10 the bar renders 44.7 design px, not 18. Verified across all 14 corner combinations at both resolutions: zero clipping, zero overflow, zero tile-chip collision.

**Live-apply — a pure style update.** The four steps reach the rendered wall through the **existing** `/api/wall` poll (5 s; no second mechanism, no push channel added). `scaleCssVars()` is a pure config → `{--feed-pct, --feed-font, --ticker-scale, --ticker-text-scale}` function, unit-tested per step; `applyWallViewTunables()` is only the DOM write and **skips any variable whose value is unchanged**, so a poll that changed nothing does zero style writes. Nothing is torn down: no tile, no `<video>`, no hls.js instance — and because hls.js runs `capLevelToPlayerSize`, a resize is an ABR level switch, not a re-attach.

One thing a size change *must* also do: **re-key the ticker crawl.** The marquee bakes a measured pixel period into a WAAPI keyframe, so resizing the strip without re-keying leaves it translating by a stale distance (a visible seam) and can leave the overflow gate on the wrong side. The ticker size is now part of `crawlSignature`, and a ticker-size change re-renders the strip so `startCrawl` re-measures. Before this, a `ticker_scale` change left the crawl wrong until the next mode rotation (≤18 s) — a pre-existing defect this change had to fix rather than quadruple.
...
**Thrash guard.** `/control/`'s sliders re-label on `input` (no network) and commit on `change` (release / keyboard commit), then **debounce 250 ms** and coalesce **per control**, so nudging two sliders inside one window produces one PUT carrying both rather than dropping the earlier edit. `withScaleSteps()` is the pure, tested core of that fold. The panel also no longer snaps a thumb the operator is holding: the 60 s channel refresh re-renders the panel, and `setSlider` now skips the `value` write for the focused range (the label still updates).

**Migration (load-time, idempotent).** The retired `feed_pct` / `feed_font` / `ticker_scale` map onto their **nearest rung**; `ticker_scale` seeds **both** ticker keys (it drove height and text through one unit). An explicitly-present new key always wins over a legacy value — the same precedence as the `render` → `outputs` migration — which is what makes a `/control/` tab left open across the deploy safe: it re-PUTs its stale snapshot, and the legacy keys lose. The legacy keys vanish by omission from the validated result's whitelist; the first PUT persists the new shape. `schema_version` **does not bump**: these are additive keys with a load-time migration (the `reload_epoch`/`outputs` precedent), and the native app hard-rejects a mismatched version — a bump would break already-installed APKs for a change native never sees.

**The honest residual.** Discretising to 10 rungs costs at most half a rung. Measured worst case across each retired range: **12.2 %** (feed width, near the old minimum), 7.0 % (feed text), 11.1 % (ticker). The values a real config is most likely to hold migrate with **zero** drift — rungs sit exactly on the old default (32 % / 1.0×) and on the feed width's old minimum (18 %). That bound is pinned by a test rather than claimed away, so a future ladder edit that widens a gap fails the build.
...
## 46. Control-panel delivery — the web surfaces were cacheable ([Unreleased], 2026-08-04)

PR-024 shipped four 1–10 sliders to `/control/` and verified the server served all four ids. **The operator opened the panel and saw three.** Ground truth was established from the wire before theorising: fetching `https://<LAN_IP>:8443/control/` over the LAN returned all four ids (`feed-width`, `feed-text`, `ticker-height`, `ticker-text`) and all four labels. **So the delivery was correct and the browser was the problem** — this was a caching defect, not a deploy or build bug.

**Root cause: the static mounts sent no `Cache-Control` at all.** Only Starlette StaticFiles' `ETag` + `Last-Modified`. With no explicit freshness a browser falls back to **heuristic caching** (RFC 9111 §4.2.2, ~10 % of the document's age) and — for ES modules and stylesheets especially — reuses the stored copy **without revalidating**, so the `ETag` is never consulted. A panel could therefore stay stale indefinitely with nothing wrong server-side, which is precisely why it was invisible from the helper's perspective.

**The fix restores a convention this repo already had and lost.** The Discord-era `_NoStoreStatic` ASGI middleware stamped `Cache-Control: no-store` keyed on **content-type**; it was removed with the `:8084` surface in PR-018, and it only ever covered that surface — so `/app/` and `/control/` on `:8443` never had it. `NoStoreStatic` is that same pattern, now wrapping **both** web mounts (the class, not just the reported incident: `/app/` had the identical exposure).

`no-store` rather than `no-cache`: these are a few KB on a LAN, so the bandwidth is irrelevant, while the failure mode it prevents — an operator staring at a UI that does not match the deployed code, with no way to tell — is expensive and very hard to diagnose from the server side, where everything looks correct.
