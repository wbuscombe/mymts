...
**Serve path (one surface).** The renderer writes `playlist.m3u8` + rolling `seg_NNNNN.ts` to a **shared `mymts-stream` volume** (renderer rw, helper **read-only**); the helper serves them at **`/api/stream/playlist.m3u8`** (correct HLS media types, no-store, an honest 503 while the renderer boots, a strict `seg_<n>.ts` match so no request escapes the dir). One LAN listener; VLC opens `https://<nas>:8443/api/stream/playlist.m3u8` (self-signed cert → accept once).

**Supervision + health.** A supervisor (`run.py`, pure policy + tests in `supervisor.py`) starts Pulse + Xvfb once, then restarts Chromium/ffmpeg with backoff and escalates a crash-loop to a stack restart. The docker HEALTHCHECK gates on **fresh segments** (not mere liveness), so a wedged ffmpeg is marked unhealthy.
