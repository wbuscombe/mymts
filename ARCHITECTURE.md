...
> **Superseded in part by §40 (2026-07).** The relative-URL routing this section describes did NOT
> work inside Discord (it rendered a black wall, then a white frame) — §40 records the two fix passes
> that replaced it: every request is now forced through `/.proxy/` explicitly, and the Activity assets
> are `no-store` + carry a JS-free failure surface. The transport model, public-origin design, token
> exchange, and status plane below are unchanged and current.
...
**The Activity (`discord-activity/`).** A static app: `new DiscordSDK(clientId)` → `ready()` → OAuth (`authorize` → exchange `code` at our token endpoint → `authenticate`) → a full-bleed `<video>` + hls.js playing the HLS through Discord's **`/.proxy/`** path. It ORIGINALLY used RELATIVE URLs (`api/stream/playlist.m3u8`) — **corrected in §40** (Discord serves the iframe at the origin ROOT, so requests are now forced through `/.proxy/` explicitly). As originally written, the playlist + its relative segment URIs were assumed to resolve against the iframe's `…/.proxy/` base automatically — Discord proxies them to the public origin. Third-party JS is vendored same-origin (the SDK esbuild-bundled to a self-contained ESM; hls.js the same pinned bundle the wall uses), `script-src 'self'`, no CDN. Testable core (`activity-core.mjs`) is browser-global-free (the OAuth wiring + the hls attach are unit-tested with fakes under `node --test`).
...
## 38. Mercury output — the real LiveKit screen_share publisher (browser SDK on the shared Xvfb) (2026-06-29)

§36's inert stub is replaced with the real publisher at the `MERCURY-WIRE-UP` seam: the wall is published into a Mercury voice channel as a simulcast `source: screen_share` track. Built + verified end-to-end against a throwaway local LiveKit dev SFU (see `docs/decisions/0003`); it goes live on Mercury as a pure config swap once the prod creds + tailnet land.

**Browser SDK, not a server SDK (note M).** Mercury's `/livekit-proxy` is hand-rolled and only proven against `livekit-client`, so a dedicated **publisher Chrome** in the renderer container runs the browser SDK (vendored UMD, same-origin, no CDN). The token is minted **server-side** in `mercury.py` (HS256, no PyJWT) with the exact publish-only grant — `roomJoin + canPublish`, `canSubscribe=false` (a default grant would pull every participant's mic to the NAS — note E), `canPublishData=false`, identity `mymts-wall-bot` (DUPLICATE_IDENTITY cleanly evicts a stale connection — note K), name `MyMTS News Wall` (note L), 24h TTL re-minted on reload (note 18). The API secret never reaches the browser; only the JWT does, via a localhost control server that also serves the publisher assets + receives status POSTs.

**Render-once via `captureStream()` of the HLS, not `getDisplayMedia` (key finding).** The intended mechanism was `getDisplayMedia` of the Xvfb framebuffer, but Chromium's X11 desktop capturer is **non-functional under the renderer's headless Xvfb**: `ScreenCapturerX11::SelectSource` returns false (the screen id maps to no XRandR monitor) and the window capturer fails too — `NotReadableError` across every flag. So the publisher **re-uses the one composite the renderer already produces**: it plays the wall's HLS (`http://mymts-helper:8082/api/stream`) in a hidden `<video>` via hls.js and publishes `video.captureStream()` — render-once (no second compositing), headless-safe, and it carries the wall's mixed audio. Trade-off: ~8–12s of HLS latency vs the live framebuffer (acceptable for an ambient wall). Two gotchas the implementation handles: the publisher Chrome's window is parked **off** the WxH framebuffer so it never pollutes the wall's `x11grab`, AND Chrome **suspends media decode in occluded windows** — so `--disable-backgrounding-occluded-windows` + `--disable-renderer-backgrounding` + `--disable-background-timer-throttling` + `--disable-features=CalculateNativeWinOcclusion` keep the off-screen renderer decoding. CORS (`Access-Control-Allow-Origin: *`) was added to `/api/stream` since the publisher fetches the HLS cross-origin.

**Forced simulcast + resilience.** Screen-share simulcast is OFF by default, so the publisher sets `simulcast: true` AND an explicit `screenShareSimulcastLayers` ladder (top = `mercury.resolution` ≤1080p, plus 720p + 360p) — else a viewer's ~320px mini-viewer pulls full res and adaptiveStream/dynacast can't downscale (note B). DNS rides Chrome's `--host-resolver-rules` (`chat.mercurychat.net` → `LIVEKIT_NODE_IP`, the tailnet IP — note H; no `/etc/hosts`/root). `dynacast` pausing the idle upstream is normal → reported `dynacast_paused`, never torn down (note C); a hard disconnect reloads with backoff, single instance (note G). The renderer status file carries the live state (`connecting | connected | publishing | dynacast_paused | reconnecting | error`) + viewer count + last error; `/control/`'s Mercury card renders them. With no creds the renderer falls back to the inert stub (zero egress). Native unchanged; PIA / other containers untouched.

## 39. Durable Cloudflare-tunnel network for the Discord Activity origin (2026-07-08)

The public Discord Activity origin (`wall.your-domain.example` → the helper's `:8084`) is served through the operator's **existing Cloudflare tunnel** (the separate `cloudflared` stack), NOT a MyMTS-owned tunnel. The original wiring was a one-off `docker network connect mymts-net cloudflared-kavita` — the cloudflared container joined *our* bridge. That survives a helper recreate but **dies if cloudflared is ever recreated** (it comes back on its own networks only), silently dropping the public origin.

The durable fix keeps the change **entirely in our compose** and never touches the cloudflared/kavita stack: `tunnel-net` (the cloudflared stack's own bridge, which cloudflared is natively on) is declared **`external: true`** in `deploy/docker-compose.nas.yml`, and `mymts-helper` is attached to **both** `mymts-net` and `tunnel-net`. Now cloudflared reaches `mymts-helper:8084` over `tunnel-net` regardless of either container's lifecycle — a `docker compose up -d` re-asserts the attachment every deploy, so it can't rot. `external: true` means compose attaches to the network but never creates, owns, or tears it down (that stays the cloudflared stack's job).

**Scope + safety.** Egress is unchanged — `tunnel-net` is an ordinary bridge out the NAS's residential WAN, never a VPN (PIA is untouched, 0 refs). Only the containers already on `tunnel-net` (cloudflared, kavita) can reach the helper there; the public internet still only sees the tunnel-mapped `:8084` (the minimal public app — Activity + `/api/discord/*` + the `/api/stream` passthrough; the full API/`/control/`/`/app/` remain LAN-only on `:8443`, which is not published to the tunnel). The prior manual `docker network connect` becomes redundant and can be dropped by the operator; leaving it is harmless.

## 40. Discord Activity — the `/.proxy/` routing fix + the white-frame cache/failure-surface hardening (2026-07)

§37's Activity did not work inside Discord. Two fix passes made it play, and hardened the failure surface so the next break is diagnosable in-frame.

**Fix 1 — every request forced through `/.proxy/` (commit 31b7802).** Root cause, in the code: the Activity built every path as a bare relative URL and relied on the iframe's base being Discord's `/.proxy/`. It isn't — `create_public_app` mounts the app at the origin ROOT, so Discord serves the iframe at `https://<app-id>.discordsays.com/`, and a relative URL resolved to `…discordsays.com/api/...` (outside `/.proxy/`), which Discord's CSP dropped before it left the sandbox. (The OAuth `fetch`es were tolerated under the `/` mapping; the hls.js manifest+segment XHR chain was not — the wall stayed black with zero stream traffic.) The app also never called the SDK's `patchUrlMappings`. The structural fix stops relying on the base: `proxied(path)` (in `activity-core.mjs`, the single source of truth) maps every request to `${origin}/.proxy${path}` when `isInsideDiscord()` (host ends `.discordsays.com`), and to the plain path standalone. hls.js is forced through the proxy with **both belts**: (a) the playlist loads via its proxied URL so relative `seg_N.ts` URIs resolve under `/.proxy/`, and (b) a custom hls.js loader (`makeProxyLoader` → `rewriteHlsUrl`) rewrites EVERY derived request — segments, child manifests, redirects, absolute URIs — back under `/.proxy/`, idempotently (blob:/data: MSE buffers untouched). The helper serves the playlist with **relative URIs only** (a test locks it) and tolerates a forwarded `/.proxy/` prefix (`_StripProxyPrefix`), so the same playlist plays for VLC/LAN too. An in-frame **diagnostics overlay** (tap the corner badge; auto-shown on a fatal error) reports the env, each OAuth stage, the resolved playlist URL, and any hls.js error's type/details/failing-URL/HTTP-status. See `docs/decisions/0005`.

**Fix 2 — cache-busting + a JS-free failure surface (commit 5354bdb).** After Fix 1 the Activity rendered a WHITE frame with no UI and no diagnostics badge. Diagnosed from the public origin (no live Discord needed): **stale Cloudflare edge cache** — `activity.css` returned `cf-cache-status: HIT` with a pre-fix `last-modified` (Cloudflare default-caches `.css` but not `.mjs`/`.html`, and the origin sent no `Cache-Control`), so a fresh `index.html`/`.mjs` loaded a stale `.css`. Hardening, so it can't recur or go silent: (1) `create_public_app` stamps `Cache-Control: no-store` on the Activity's HTML/CSS/JS (a `_NoStoreStatic` ASGI middleware, keyed on content-type — the HLS media keeps its own headers); (2) `index.html` is served with the running helper's git SHA injected into a `__MYMTS_BUILD_SHA__` placeholder (a `<meta>` + comment), so "which build is Discord running?" is answerable by view-source forever; (3) `index.html` carries a **JS-free failure surface** — an inline dark background FIRST (never a white void), an inline `window.onerror`/`unhandledrejection` handler FIRST in `<head>` that writes the error on-screen (runs even if the module fails to load), and a static "loading…" state + `<noscript>` (CSP adds `'unsafe-inline'` for script/style so the bootstrap renders; `connect-src 'self'` stays strict). The one residual manual step is a one-time Cloudflare purge of the already-cached stale copy; after this it can't recur (no-store). Discord-Activity + helper only; native untouched, Mercury inert, renderer unchanged.
