...
| Endpoint | Returns |
|---|---|
| `GET /api/wall` | the current wall config (or a server default when none stored), tagged `{…, stored}` — `{schema_version, layout:{rows,cols}, preset, audible_cell, reload_epoch, render:{resolution}, feed_pct, feed_font, ticker_scale, cells:[{channel, subtitles, reload}], stored}` |
| `PUT /api/wall` | **partial-merge (PATCH)**: a field PRESENT in the body overrides; an OMITTED field is PRESERVED from storage (so `/app/` can't clobber a field it doesn't send). Validation runs on the MERGED result against the live slug-set (+ the radar slugs); reload counters are clamped monotonic; `422` with a field-specific reason on a contract violation. See §30 |
