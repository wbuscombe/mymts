...
- Pre-commit secret-scanning (gitleaks or equivalent) runs locally; CI re-runs it as a gate.
...
- Advisory scanning (`npm audit`, `pip-audit`, equivalent for Kotlin/Gradle) runs in CI. Fix-available advisories block; no-fix advisories annotate and post to the operator's notification channel.
