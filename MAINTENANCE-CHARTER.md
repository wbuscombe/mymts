...
| **Renderer tests** (supervisor policy + encode-pipeline fan-out + self-heal + config-read churn fix) | `[ENFORCED]` — `ci.yml` (`python -m unittest -s renderer`) | the renderer fan-out/self-heal |
