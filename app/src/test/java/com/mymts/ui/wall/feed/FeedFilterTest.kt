...
class FeedFilterTest {
    ...
    @Test fun `hidden source is filtered out, others kept`() {
        ...
        val out = FeedListBuilder.applyFilters(items, setOf("Reason"), FeedRecency.All, NOW)
    ...
    @Test fun `denylist match is case-insensitive`() {
        ...
        val out = FeedListBuilder.applyFilters(items, setOf("bbc world"), FeedRecency.All, NOW)
    ...
    @Test fun `empty denylist keeps everything (new sources show by default)`() {
        ...
        val out = FeedListBuilder.applyFilters(items, emptySet(), FeedRecency.All, NOW)
        assertEquals(2, out.size)
    }

    @Test fun `blank source hidden via the Unknown source bucket label`() {
        val items = listOf(item(1, "", 5), item(2, "BBC", 5))
        val out = FeedListBuilder.applyFilters(items, setOf("Unknown source"), FeedRecency.All, NOW)
    ...
    @Test fun `recency All keeps every item regardless of age`() {
        ...
        assertEquals(2, FeedListBuilder.applyFilters(items, emptySet(), FeedRecency.All, NOW).size)
    }
    ...
    @Test fun `recency window drops items older than the window`() {
        ...
        assertEquals(setOf(1L), FeedListBuilder.applyFilters(items, emptySet(), FeedRecency.Hour, NOW).map { it.id }.toSet())
        assertEquals(setOf(1L, 2L), FeedListBuilder.applyFilters(items, emptySet(), FeedRecency.SixHours, NOW).map { it.id }.toSet())
        assertEquals(setOf(1L, 2L, 3L), FeedListBuilder.applyFilters(items, emptySet(), FeedRecency.Day, NOW).map { it.id }.toSet())
    }

    @Test fun `item with no timestamp is kept under All but dropped under a bounded window`() {
        val noTs = item(9, "BBC", null)
        assertTrue(FeedListBuilder.applyFilters(listOf(noTs), emptySet(), FeedRecency.All, NOW).isNotEmpty())
        assertTrue(FeedListBuilder.applyFilters(listOf(noTs), emptySet(), FeedRecency.Day, NOW).isEmpty())
    }
    ...
    @Test fun `source + recency compose`() {
        ...
        val out = FeedListBuilder.applyFilters(items, setOf("Reason"), FeedRecency.Hour, NOW)
