...
| `scripts/deploy-*.sh`, `probe-tile-count.sh`, `soak.sh` | box/helper LAN IPs + ssh host alias | env-var-overridable with placeholder defaults + a gitignored `scripts/deploy.local.env` for the operator's real values |
